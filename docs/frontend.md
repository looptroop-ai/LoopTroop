...
`LogProvider` treats the server-side execution-log projection as durable truth. SSE-delivered rows merge into a bounded in-memory overlay immediately, by stable entry identity, so phase and full-log views stay live without waiting for file persistence. Restored/history queries request the newest 250 matching projected rows, merge the page with that live overlay in one Map-indexed pass, and fetch older cursors only when the user scrolls upward. Large phase and lifecycle timelines virtualize log rows and bead/phase delimiters with `react-virtuoso`; **Copy all** uses the complete-history export route rather than the rendered window. The browser no longer writes durable log snapshots to `localStorage`; only the SSE replay cursor remains browser-persistent. None of these history/read changes replay restored rows through live persistence or SSE broadcasting.
...
AI log rows use consistent visible headers for prompts, reasoning, tools, and output. In combined AI views these render as `[PROMPT-<model-id>]`, `[THINKING-<model-id>]`, `[TOOL-<model-id>]`, and `[OUTPUT-<model-id>]`, using the complete configured model ID (including provider and router segments). Truncated streaming previews render that header once above the retained tail instead of repeating it inside the response. Persisted canonical tags remain unchanged so historical records and internal extraction continue to work.
...
Failed execution setup plan and runtime reports keep `modelOutput` out of the structured details/body. When available, that model output and any `rawAttempts` are exposed through Raw tab variants so failure diagnostics remain inspectable without presenting rejected setup text as accepted plan content. Honest blocked setup profiles remain diagnostic-only: the structured view can explain the blocker, but LoopTroop does not publish them as the reusable runtime profile. The visible workflow error names the failed setup area and includes the agent's concise blocker explanation. A setup attempt that still returns only progress prose after two same-session continuations reports that the agent stopped before completing preparation; missing-marker details remain in Raw diagnostics.

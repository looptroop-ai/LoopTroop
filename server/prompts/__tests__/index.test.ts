...
describe.concurrent('structured prompt hardening', () => {
  ...
  it('requires the bead subset schema consistently in draft and refine prompts', () => {
    ...
    expect(PROM20.outputFormat).toContain('For a structured shell command whose `script` contains regex backslashes')
    ...
    expect(draftPrompt).toContain('For a structured shell command whose `script` contains regex backslashes')
  ...
  it('defines execution setup prompts with explicit workspace setup rules', () => {
    ...
    expect(setupPlanPrompt).toContain('Backend-Owned Fields')
    expect(setupPlanPrompt).toContain('ticket-locked project Git-hook policy')
    expect(setupPlanPrompt).toContain('locked policy is copied exactly')
    ...
    expect(setupPlanPrompt).toContain('Project Agnosticism')
    expect(setupPlanPrompt).toContain('Do not assume a programming language, build system, package manager, shell, operating system')
    ...
    expect(setupPlanRegeneratePrompt).toContain('Backend-Authoritative Hook Policy')
    expect(setupPlanRegeneratePrompt).toContain('Project Agnosticism')
    expect(setupPlanRegeneratePrompt).toContain('Do not assume a programming language, build system, package manager, shell, operating system')
    ...
    expect(setupPrompt).toContain('Project Agnosticism')
    expect(setupPrompt).toContain('Repository-Native Setup')
    expect(setupPrompt).toContain('Temporary Scope and Safety')
    expect(setupPrompt).toContain('Do not use privileged or global installation')
    expect(setupPrompt).toContain('Missing Tool Recovery')
    expect(setupPrompt).toContain('try at least two distinct safe user-space strategies')
    expect(setupPrompt).toContain('Wrapper creation, cache inspection, PATH edits, and probes are not provisioning attempts')
    expect(setupPrompt).toContain('consult official release metadata when needed')
    expect(setupPrompt).not.toContain('OpenCode `websearch`')
    expect(setupPrompt).not.toContain('`webfetch` for official release/download metadata')
    expect(setupPrompt).not.toContain('Provisioning Examples, Non-Exhaustive')
    expect(setupPrompt).not.toContain('For Node')
    expect(setupPrompt).not.toContain('for Python')
    expect(setupPrompt).not.toContain('for JavaScript runtimes such as Deno or Bun')
    ...
    expect(setupPrompt).not.toContain('.ticket/runtime/execution-setup/env.sh')
    expect(setupPrompt).toContain('Runtime Environment')
    expect(setupPrompt).toContain('"runtime_environment"')
    expect(setupPrompt).toContain('"mode":"process"')
    expect(setupPrompt).toContain('Temporary Scope and Safety')
    expect(setupPrompt).toContain('Gitignore Suggestions')
    expect(setupPrompt).toContain('do not edit `.gitignore` during setup')
    expect(setupPrompt).toContain('Do not implement ticket features')
    expect(setupPrompt).toContain('do not leave those changes behind')
    expect(setupPrompt).toContain('Honest Outcome')
    expect(setupPrompt).toContain('Return `ready` only when every setup check passes')
    expect(setupPrompt).toContain('required tool is `failed` or `not_provisionable`')
    expect(setupPrompt).toContain('Completion')
    expect(setupPrompt).toContain('A progress update such as "provisioning the toolchain" is never a final answer')
    expect(setupPrompt).toContain('do not stop at a statement of intent')
    expect(PROM_EXECUTION_SETUP.outputFormat).toContain('"status": "<ready|blocked>"')
    expect(PROM_EXECUTION_SETUP.outputFormat).toContain('"tooling": "<pass|fail>"')
    ...
    expect(setupPrompt).toContain('Backend-Owned Runtime Fields')
    ...
    expect(setupNotePrompt).toContain('Project Agnosticism')
    expect(setupNotePrompt).toContain('another actionable approach remains or no safe preparation path exists')
    expect(setupNotePrompt).toContain('honest structured `ready` or `blocked` outcome')
    expect(setupNotePrompt).toContain('progress-only response is unfinished work')
  })
  ...
  it('PROM_CODING includes completion instructions, bead context guidance, and self-check', () => {
    ...
    expect(prompt).toContain('Prepared Runtime Launcher')
    expect(prompt).toContain('command-launcher')
    expect(prompt).toContain('host_context.preferredShell')
    ...
    expect(prompt).toContain('planned test commands as starting guidance')
    expect(prompt).toContain('smallest appropriate automated checks')
    expect(prompt).toContain('does not independently rerun a frozen copy of the bead test commands')
    expect(prompt).toContain('Return `done` only after you have run the applicable checks')
    expect(PROM_CODING.contextInputs).toEqual(['bead_data', 'bead_notes'])
  })

  it('tells final-test generation to reuse the structured prepared runtime environment', () => {
    const prompt = buildPromptFromTemplate(PROM52, [])

    expect(prompt).toContain('Execution Setup Reference')
    expect(prompt).toContain('.ticket/runtime/execution-setup-profile.json')
    expect(prompt).toContain('structured PATH additions and variables')
    expect(prompt).toContain('"mode":"process"')
    expect(prompt).toContain('current host context')
  })
  ...
  it('requires Manual QA fix-bead generation to inspect the repository and return every full candidate', () => {
    ...
    expect(prompt).toContain('acceptance criteria, automated test scenarios, appropriate verification command guidance')
    expect(prompt).toContain('Detailed tests and acceptance criteria do not each require a matching command')
  ...
  it('keeps PROM4 and PROM52 explicit about marker-only structured output', () => {
    ...
    expect(PROM52.contextInputs).toEqual(['ticket_details', 'prd', 'beads', 'final_test_notes', 'host_context'])
  })
})

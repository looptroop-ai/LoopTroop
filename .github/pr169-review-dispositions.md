...
| 5714101041 | looptroop-ai | mixed | Drain scope, model retry codes, DEBUG interleaving, applied snapshot generations, and full-ticket patch fields are fixed. Native helper cleanup is in `baa491b9`. Create-success editing remains held for the user decision. |
| 5714233224 | looptroop-ai | correct / fixed | Deferred fold, older-page requests, and drain state are keyed to the serialized query scope; a scope-change regression proves the new scope fetches its own older page. |
...
| 5714296137 | looptroop-ai | deferred / product decision | A successful project create followed by newer typing has two valid UX resolutions. PR169 preserves the draft and does not silently select a behavior before the user decides. |
...
| 5714879877 | looptroop-ai | correct / fixed | Drain ownership is scoped per query key with independent caller cancellation, and `drainError` clears on scope changes. The historical-log focused suite passes. |
...
| Comment ID | Author | Disposition | Evidence / decision |
| --- | --- | --- | --- |
| 4036449249 | qodo-code-review | correct / fixed | `fetchAllOlder` and `requestOlderPage` now bind in-flight work to the current query scope; the scope-switch drain regression passes. |

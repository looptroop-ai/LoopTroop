...
export const PROM21: PromptTemplate = {
  ...
  instructions: [
    ...
    'Command Feasibility: Treat test commands that are unsupported by the repository evidence as correctness defects. Do not reward generic package-manager commands that do not match the relevant workspace manifests, scripts, or checked-in task runners.',
...
export const PROM25: PromptTemplate = {
  ...
  instructions: [
    ...
    'Command Compatibility Invariant: `testCommands` have already passed repository-evidence validation. Preserve them byte-for-byte; do not replace them with commands from another ecosystem during expansion.',
...
export const PROM_EXECUTION_SETUP_PLAN: PromptTemplate = {
  ...
  instructions: [
    ...
    'Bead Command Audit: Cross-check every bead `testCommands` entry against repository manifests, build files, scripts, checked-in task runners, and its effective working directory. Do not copy or legitimize an unsupported bead command. A globally installed launcher alone does not prove that its ecosystem belongs to this repository.',
...
export const PROM_EXECUTION_SETUP_PLAN_REGENERATE: PromptTemplate = {
  ...
  instructions: [
    ...
    'Revalidate bead and project command compatibility while revising. Do not retain an unsupported launcher merely because it appeared in the previous plan.',

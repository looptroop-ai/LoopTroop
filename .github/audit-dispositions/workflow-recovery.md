...
| W09/W10 | PASS, final pending | Council stop-before-settlement uses a bounded session-publication wait and shared cancellation cleanup confirms the remote stop before cleanup, continuation, skip-all advancement, or pending-marker promotion. A session published after the deadline is stopped by the prompt callback-failure path before its error is rethrown. Adapter lookup accepts only exact HTTP 404 evidence; a transport 500 cannot be masked by nested business data that says 404, and message-only 404 text remains unconfirmed. False, thrown, and unverified stops remain retryable, while failed CANCELED cleanup persists the non-expiring `.ticket/runtime/cancellation-pending.json` marker and retries without re-entering coding. The separate `runtime/opencode-pending-sessions.json` marker is the durable session-ID ownership evidence; the cancellation marker records a stop request only. | No live model or lifecycle run. |
...
Only a confirmed remote stop releases ownership. SQLite ownership may fall back to the durable session-ownership marker at `runtime/opencode-pending-sessions.json` during startup recovery. Cancellation also records the private, non-expiring `.ticket/runtime/cancellation-pending.json` runtime artifact before cleanup; an absent marker means no pending stop, while malformed or unreadable marker content fails closed and blocks coding. The cancellation marker records a stop request but cannot identify or recover a remote session. Cleanup removes it only through the contained ticket-file boundary after terminal cleanup. If both storage layers are unavailable, only the current process guard remains, so a restart cannot claim recovery. Do not describe a database reopen as a process restart or claim that a simultaneous database and marker failure is recoverable across restart.

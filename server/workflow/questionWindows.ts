...
export async function clearTicketWindows(
  ...
  reason: string,
  isCleanupStillOwned: () => boolean = () => true,
): Promise<boolean> {
  const timers = timersByTicket.get(ticketId)
  let allFinished = true
  for (const timer of [...(timers?.values() ?? [])]) {
    if (!isCleanupStillOwned()) return false
    disarm(timer)
    const doomed = pendingRequests(timer)
    ...
    for (const record of doomed) {
      if (!isCleanupStillOwned()) return false
      const finished = await rejectRecord(timer, record, 'system', resolution, reason, siblingIds)
      if (!isCleanupStillOwned()) return false
      if (!finished) {
        allFinished = false
      }
  ...
  if (!allFinished || !isCleanupStillOwned()) return false
  const currentTimers = timersByTicket.get(ticketId)
  // A newer run may have attached a question while an older rejection was
  // awaiting OpenCode. Never delete its map or bookkeeping at the old run's
  // destructive boundary.
  if (currentTimers && currentTimers !== timers) return false
  if (currentTimers && [...currentTimers.values()].some((timer) => pendingRequests(timer).length > 0)) {
    return false
  }
  if (currentTimers === timers) timersByTicket.delete(ticketId)

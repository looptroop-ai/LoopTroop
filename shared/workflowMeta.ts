...
const WORKFLOW_PHASE_DETAILS = {
  ...
  WAITING_INTERVIEW_ANSWERS: {
    ...
    steps: [
      'Question batch: The workspace shows the current set of pending questions, including type, context, and prior answered or skipped state. Every answer or skip submission carries a positive batch identity, so a stale tab is rejected instead of overwriting a newer batch.',
      ...
      'Batch submission: Submitting writes the latest answers and skip decisions into the interview session and canonical artifact. A durable claim covers generation and answer edits, so a released or stale claim cannot overwrite a newer snapshot. The model may inspect a small repository area read-only when it needs to prepare the next batch, but your product decisions remain authoritative.',
      'Timeout and stop recovery: If a delayed timeout or remote stop cannot be confirmed, the current batch remains visible and retryable. Only a confirmed stop can promote the exact pending marker and release the batch for the next action.',
  ...
  WAITING_INTERVIEW_APPROVAL: {
    overview: 'The interview is ready for review. Nothing moves forward until you approve it. You can inspect the structured view or raw YAML. Draft edits autosave with a visible last-save status, but they do not update the authoritative interview artifact until you explicitly Save. A dirty draft keeps the hash of the document it loaded, so a missing baseline fails closed and a stale baseline returns a conflict instead of silently overwriting newer content.',
    steps: [
      'Review modes: You can switch between a structured question-and-answer view and the raw YAML.',
      'Editing Answers And Draft Autosave: You can adjust any answer text, change skip decisions, or modify the raw YAML directly. Marking an answer skipped takes an optional reason, and answering the question clears it. Draft changes autosave between view switches and reloads while retaining the original loaded content hash. The visible Draft autosave on indicator reports pending, saving, saved, conflict, or failure state plus the last server-acknowledged save time. Missing save baselines return HTTP 428; stale baselines return HTTP 409, and a failed save keeps the draft unsaved for retry.',
    ...
    notes: [
      ...
      'A persisted dirty draft is not silently rebased onto newer remote content. A failed or best-effort leaving flush is not confirmation that the server saved the latest text.',
  ...
  WAITING_PRD_APPROVAL: {
    overview: 'The latest PRD candidate is ready for review before beads planning starts. Draft edits autosave with a visible last-save status, but they do not update the authoritative PRD until you explicitly Save. A dirty draft keeps the hash of the document it loaded, so a missing baseline fails closed and a stale baseline returns a conflict instead of silently overwriting newer content. You can also inspect the read-only Full Answers context and any remaining coverage warnings before approval.',
    steps: [
      ...
      'Editing And Draft Autosave: You can edit any section of the PRD. Draft changes autosave between view switches and reloads while retaining the original loaded content hash. The visible Draft autosave on indicator reports pending, saving, saved, conflict, or failure state plus the last server-acknowledged save time. Draft autosave does not update the authoritative PRD; explicit Save applies the draft and records a user-edit receipt. Missing save baselines return HTTP 428; stale baselines return HTTP 409, and a failed save keeps the draft unsaved for retry. If you save a post-approval edit before PRE_FLIGHT_CHECK, LoopTroop archives the current approved PRD and restarts downstream planning from the edited version.',
  ...
  CODING: {
    overview: 'LoopTroop runs approved beads one at a time. The coding agent uses planned commands as guidance, runs the smallest sensible bead-scoped checks, and must return a structured all-pass completion marker. LoopTroop validates that marker without rerunning a frozen command list on its own. Automatic bead-response continuations use the configured `maxIterations` cap within each bead iteration when it is finite; `0` means unlimited for that automatic path. User-facing Continue across workflow phases is separate. Deadline expiry and unresumable restarts go through Ralph reset-and-retry recovery. Local finalization failures become manual Retry or Cancel blocks. Bounded Git resets and commits preserve opaque path names and keep generated runtime files out of candidate commits. A conflicting OpenCode step-cap restore refuses a destructive reset while preserving the edited root config and its sidecar; a later bead can continue without a fresh cap when no reset is needed, with that root config excluded from delivery. When all beads are done, Final Testing is still the required whole-ticket gate.',
    steps: [
      'Bead Selection and Tracker Update: LoopTroop reads the authoritative bead tracker, finds all runnable beads, meaning status `pending` with every `blocked_by` entry already done, sorts them by `priority` ascending, and selects the first one. It prepares that bead for execution and updates ticket progress only after its reset checkpoint is recorded.',
      'Bead Start Commit Recording: Before publishing the bead as `in_progress`, LoopTroop records the current git HEAD SHA of the worktree as `beadStartCommit` and stores it in the bead tracker. This is the reset anchor if a later iteration fails. If the HEAD read or recording fails, the bead stays `pending` and this execution attempt stops before the agent runs.',
      'Restart Recovery: If CODING resumes with an interrupted `in_progress` bead, LoopTroop first checks for an explicitly preserved continuation and then the latest `bead_execution:{beadId}` artifact. It reuses the exact preserved session or finalizes a checkpoint only when bead id, iteration, `startedAt`, `updatedAt`, and `beadStartCommit` match current durable state. Otherwise the interrupted attempt gets a deterministic Failed Iteration Note and attempts a safe reset to its start snapshot. A conflicting OpenCode step-cap marker can refuse that destructive reset while preserving the edited config and sidecar; when recovery is safe, the bead advances to the next iteration and returns to `pending`, and the scheduler starts that replacement with a fresh configured deadline.',
      ...
      'Failure Path, Worktree Reset and Status Rollback: After appending the Failed Iteration Note, LoopTroop attempts a safe reset of the worktree to `beadStartCommit`, preserves `.ticket` artifacts, returns the bead to `pending`, abandons the active session, clears stale continuation state, and increments the outer iteration when recovery succeeds. A valid OpenCode step-cap sidecar that shows the current root config conflicts with the cap makes this destructive reset refuse instead of overwriting the file; the edited config and sidecar remain available. Ordinary capped runs still reset and retry normally, and a later bead can run without a new cap when it does not need that reset, while the valid pending marker keeps the root config out of bead and final staging. Reset and cleanup wait for bounded process-tree termination; if safe recovery cannot complete, the bead remains blocked rather than continuing against unknown state. Late output from the expired attempt cannot finalize the bead.',
    ...
    notes: [
      ...
      'The `beadStartCommit` is recorded before the bead becomes `in_progress`. If Git cannot record it, the bead stays `pending`, no execution call starts, and the next attempt can retry the checkpoint.',
      ...
      'OpenCode step-cap recovery is conditional: ordinary capped runs reapply the cap after a reset and restore it on exit. A conflicting current file refuses only destructive reset or recovery that could overwrite it, preserving the edited bytes and valid sidecar. If the sidecar is missing after a restart, ownership cannot be proven and is not guessed. Applying a cap does not write a common Git exclude rule; while a valid marker exists, commit selection excludes the temporary root config. Filesystem-equivalent casing follows the actual worktree paths, and native Windows/macOS equivalent-case behavior is not verified here.',
  ...
  WAITING_MANUAL_QA: {
    overview: 'LoopTroop waits in the interactive Manual QA workspace while the user runs the application, records results and evidence, and verifies the generated checklist. Every item starts as Pending with no result-specific fields. Draft work autosaves with a visible last-save time. Evidence storage keeps a strict no-symlink policy, event diagnostics retain invalid lines, evidence locking uses a persistent transaction database, and only Submit or Skip finishes the round.',
    steps: [
  ...
  INTEGRATING_CHANGES: {
    ...
    notes: [
      ...
      'Normal integration continues according to the selected hook policy. The durable marker guard applies to the protected validation path; it does not claim that unrelated hook commands are harmless or that every operation is fully transactional.',
    ],
  },
  ...
  CANCELED: {
    overview: 'The ticket was stopped by user action before normal completion, active remote work was confirmed stopped, and it is now in a terminal canceled state. If a stop cannot be confirmed, the ticket stays retryable instead of entering this terminal state. By default, the work and artifacts created up to that point are preserved. At cancellation time, the user can optionally choose to delete AI-generated artifacts such as interview results, PRD drafts, beads plans, worktree code, and optionally the execution logs.',
    steps: [
      ...
      'Active Session Cleanup: If AI sessions were still running when cancellation happened, for example during a council phase or coding, LoopTroop treats them as stopped only after remote abort confirmation. A false, thrown, or unverified abort leaves ownership visible and retryable, so cleanup and deletion do not advance.',
  ...
  BLOCKED_ERROR: {
    ...
    steps: [
      ...
      'Decision Point: The live view shows only recovery actions the server advertises for this occurrence: Retry and Cancel when available, Continue only for an eligible preserved session, setup-plan editing only when `edit_execution_setup_plan` is advertised for a failed runtime-setup phase, and note-bearing Retry only when the server advertises retry for a status that supports that note. A setup-approval error does not itself grant setup-plan editing or note-bearing retry. For coding, an advertised note is appended to User Retry Notes after safe bead recovery. Finalization failures already have their own concise Finalization Failure Note and stay manual Retry or Cancel blocks. Ralph is used only for coding-iteration failures such as the workflow deadline.',
    ],
    outputs: [
    ...
    transitions: [
      'Retry → Previous Status: Retry archives the active phase attempt with `manual_retry_after_blocked_error`, creates the next active attempt, and returns the workflow to the previously blocked status for every non-implementation phase. CODING keeps its special failed-bead reset when recovery is safe before re-entering and does not create phase versions. For a live CODING block, Retry with extra note... performs the same safe recovery and appends a timestamped entry only to User Retry Notes after the bead can be identified and reset. OpenCode retry-budget or provider and session blocks may use Continue instead when the session is still addressable by exact id.',
      ...
      'Continue → Previous Status: Continue is available only for eligible OpenCode or provider interruptions with a preserved active session addressable by exact id, including transient provider stalls, provider or session timeouts, selected 5xx or 529 and 408 or 429 responses, transport failures, and `HTTP 402 Payment Required` blocks that can be cleared outside the session. It is not available for CODING workflow-owned iteration timeouts, which attempt safe resets and fresh retries until the bead budget is exhausted; a recovery conflict can block instead of resetting. Eligible sessions survive backend, OpenCode, WSL, OS, and machine restarts when the unresolved error occurrence, previous phase, and exact remote session still match. Temporary verification failures preserve the active record for a later check, while only confirmed absence or stale ownership abandons it. Continue returns to the previous status without archiving or creating phase attempts and sends exactly `continue please` into that same session. For CODING provider interruptions, the blocked view treats the bead as paused instead of actively counting down. After Continue, LoopTroop starts a fresh per-iteration timeout window while reusing the preserved OpenCode session.',
...
const BASE_WORKFLOW_PHASES = [
  ...
  },
  {
    ...
    label: 'Interviewing',
    description: 'Answer the interview questions that will shape the PRD, or skip them with an optional reason the answering model gets to read. Every submission carries a positive batch identity, so stale tabs are rejected instead of overwriting a newer batch. Draft answers autosave with visible state and last-save time. Non-final submissions can load another batch, and coverage may send the ticket back here with focused follow-up questions.',
  ...
  },
  {
    ...
    description: 'Review and approve the final interview before PRD drafting starts, including any answers you want to skip and why. Draft edits autosave with visible state and last-save time, while the loaded content hash guards raw and structured saves: missing baselines fail closed and stale baselines return a conflict; explicit Save is required to update the authoritative artifact. Approval then locks this interview as the source of truth for PRD drafting.',
  ...
  },
  {
    ...
    description: 'LoopTroop is preparing the current host for this ticket with the approved setup plan. The phase must finish with an honest Ready or Blocked result before coding can start. Any temporary tooling, inputs, and environment changes are recorded in the reusable runtime profile, while setup recovery uses bounded Git cleanup.',
  ...
  },
  {
    ...
    description: 'LoopTroop is implementing the ticket one bead at a time. Each bead records its reset checkpoint before it becomes `in_progress` and must finish its own checks before the workflow moves to the next one. Automatic bead-response continuations within each bead iteration follow the finite `maxIterations` cap when configured, while `0` means unlimited for that path; user-facing Continue across phases is separate. Success creates bead-level execution history, while bounded Git resets and commits preserve opaque paths. A conflicting OpenCode step-cap restore can stop a destructive reset while preserving the edited config and sidecar; a later bead may continue without a fresh cap when no reset is needed, with valid marker evidence keeping the root config out of delivery.',
  ...
  },
  {
    ...
    description: 'LoopTroop is turning the bead commits into one review-ready candidate commit while applying the approved hook policy and final file audit. Git path records stay opaque, local mutations are bounded, and shared generated-file exclusions keep local noise out of the candidate. Normal integration proceeds under the selected policy; if an interrupted protected hook validation cannot safely account for unknown untracked additions, recovery refuses reentry and leaves those paths intact until resolved.',
  ...
  },
  {
    id: 'CANCELED',
    label: 'Canceled',
    description: 'The ticket was canceled after active remote work was confirmed stopped. No more automation will run, but existing artifacts stay available unless you chose to delete them at cancel time. If a stop cannot be confirmed, the ticket remains retryable instead. This terminal state preserves the work completed so far for later review.',
  ...
  },
  {
    id: 'BLOCKED_ERROR',
    label: 'Error (reason)',
    description: 'A phase failed and LoopTroop is waiting for your recovery decision. The ticket keeps the error context, logs, and the exact phase that needs retry or continuation. The detailed view explains what failed and which ticket recovery actions are actually available. The live controls are limited to actions advertised by the server for this occurrence, so setup errors do not invent edit-plan or note-bearing retry controls.',

...
const WORKFLOW_PHASE_DETAILS = {
  ...
  PREPARING_EXECUTION_ENV: {
    overview: 'LoopTroop runs a dedicated execution setup phase after setup-plan approval and before coding. It materializes approved workspace inputs, verifies the approved readiness assessment, provisions missing runtime tooling only under approved temporary paths, validates and automatically recovers the canonical runtime wrapper, runs tooling and ordered repository-level workspace probes through the prepared environment, and applies the approved Git-hook policy. Setup remains language, build-system, shell, and operating-system agnostic by following repository evidence. The agent must finish with an honest structured Ready or Blocked result. One Execution Setup Timeout deadline bounds all active work in each attempt; every genuine retry starts with a fresh full budget. After automatic retries end, the live blocked view can send one user note to the preserved setup session for one additional manual attempt.',
    steps: [
      ...
      'Wrapper Recovery And Routing: LoopTroop prefers the profile-declared command wrapper, but can recover the canonical executable `.ticket/runtime/execution-setup/run` from disk when the model omitted its declaration. It records that repair in the accepted profile and routes tooling probes, workspace probes, explicit hook checks, coding verification, and later project commands through a non-login nested shell so the prepared environment is not overwritten.',
      ...
      'Structured Validation: LoopTroop parses the result via a strict marker/schema contract and rejects contradictory outcomes such as Ready with a failed check or Blocked with every check passing. If a completed result has the wrong marker or schema, it may use the ticket\'s configured Structured Output Retries count with a continued-session repair prompt, and records rejected/accepted raw attempts on the setup report. Ready profiles are still rejected when a declared command wrapper is missing or unusable, declared tooling probes fail, or wrapper/project command families are declared without probes.',
...
const BASE_WORKFLOW_PHASES: WorkflowPhaseMeta[] = [
  ...
  },
  {
    ...
    description: 'Materializing approved workspace inputs and verifying the prepared runtime through repository-grounded probes. The setup agent must finish with an honest Ready or Blocked result; progress-only replies receive two same-session continuations before an attempt fails. Actionable blockers retry, no-safe-path blockers stop, and the user can still send one note or rewind to edit the setup plan.',

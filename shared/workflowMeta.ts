...
const WORKFLOW_PHASE_DETAILS = {
  ...
  CODING: {
    ...
    notes: [
      ...
      'The `beadStartCommit` is recorded before a bead becomes `in_progress`. If git cannot record it, the bead remains pending and a Retry can reattempt the checkpoint; an already-started bead without an anchor still fails closed during reset recovery.',
  ...
  CANCELED: {
    ...
    steps: [
      ...
      'Active Session Cleanup: If AI sessions were still running when cancellation happened, for example during a council phase or coding, LoopTroop performs cleanup only after the remote stop is confirmed. A false, thrown, or unverified stop leaves ownership visible and retries the cleanup while the ticket remains canceled, so coding cannot re-enter with the old controller. When persisted, the private `.ticket/runtime/cancellation-pending.json` marker is written before cleanup, remains across a restart, and is removed only after confirmed terminal cleanup; malformed or unreadable marker content blocks coding. This marker records a stop request, not a remote session ID; durable session ownership uses `runtime/opencode-pending-sessions.json`.',
...
const BASE_WORKFLOW_PHASES = [
  ...
  },
  {
    ...
    description: 'LoopTroop is implementing the ticket one bead at a time. Each bead must finish its own checks before the workflow moves to the next one. Automatic bead-response continuations within each bead iteration follow the finite `maxIterations` cap when configured, while `0` means unlimited for that automatic path; user-facing Continue across phases is separate. Success creates bead-level execution history, while bounded Git resets and commits preserve opaque paths and failures reset safely or pause for manual recovery.',

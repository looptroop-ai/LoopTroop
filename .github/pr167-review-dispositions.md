...
## Final cancellation-cleanup review (2026-09-18)

Greptile `4046573568` identified a valid P1 race: a stale cancellation pass
could await an OpenCode question rejection while a CODING Retry opened a new
question, then delete the shared timer map. `clearTicketWindows` now checks an
actor/marker ownership predicate around every await and refuses to delete newer
timer state; the runner supplies that predicate. The question-window regression
keeps the newer request visible after the hand-off.
Greptile `4046691316` found the follow-up generation race: a later Cancel could
re-mark the ticket while the old pass was still awaiting. Cancellation cleanup
now captures a generation and requires it to remain current around every
destructive await; the lifecycle regression verifies that clearing and
re-marking invalidates the old generation.
...
| 4046466505 | Correct, fixed | Runtime-scoped admission now keeps per-runtime ticket counts while `getTotalClientCount()` remains process-wide, so the global 100-stream cap cannot be multiplied by embedded runtimes. A broadcaster regression covers both scope counts and the shared total. |

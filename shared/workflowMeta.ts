...
const WORKFLOW_PHASE_DETAILS = {
  ...
  CODING: {
    overview: 'LoopTroop runs approved beads one at a time. The coding agent uses planned commands as guidance, runs the smallest sensible bead-scoped checks, and must return a structured all-pass completion marker. LoopTroop validates that marker without rerunning a frozen command list on its own. Deadline expiry and unresumable restarts go through Ralph reset-and-retry recovery. Local finalization failures become manual Retry or Cancel blocks. Bounded Git resets and commits preserve opaque path names and keep generated runtime files out of candidate commits. When all beads are done, Final Testing is still the required whole-ticket gate.',
    steps: [
...
const BASE_WORKFLOW_PHASES = [
  ...
  },
  {
    ...
    description: 'LoopTroop is implementing the ticket one bead at a time. Each bead must finish its own checks before the workflow moves to the next one. Success creates bead-level execution history, while bounded Git resets and commits preserve opaque paths and failures reset safely or pause for manual recovery.',

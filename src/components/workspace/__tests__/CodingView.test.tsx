...
describe('CodingView', () => {
  ...
  it('fetches full bead data even when runtime bead placeholders already exist', async () => {
    ...
    await waitFor(() => {
      expect(fetchSpy).toHaveBeenCalledWith(
        `/api/tickets/${encodeURIComponent('1:TEST-1')}/beads`,
  ...
  it('does not show or fetch bead controls outside the implementing phase', async () => {
    ...
    await waitFor(() => {
      expect(fetchSpy.mock.calls.some(([url]: [string, ...unknown[]]) => url === `/api/files/${encodeURIComponent('1:TEST-1')}/prd`)).toBe(true)
    })
    expect(

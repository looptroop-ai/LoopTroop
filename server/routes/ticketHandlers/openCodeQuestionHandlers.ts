...
export async function handleReplyOpenCodeQuestion(c: Context) {
  ...
  // send a second verdict for a question that already has one.
  if (!claimRequestForReply(ticketId, question.sessionId, requestId)) {
  ...
  try {
    ...
    markRequestReplied(ticketId, question.sessionId, requestId)
    ...
    releaseRequestClaim(ticketId, question.sessionId, requestId)
    const message = getErrorMessage(err)
...
export async function handleRejectOpenCodeQuestion(c: Context) {
  ...
  try {
    ...
    markRequestSkipped(ticketId, question.sessionId, requestId, reason)

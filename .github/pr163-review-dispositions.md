...
temporary `/tmp` report is treated as proof. `held` means the implementation
choice is intentionally waiting on the root/user overflow-UID decision.
...
| `4030919519`, `4030924389`, `4030998343`, `5714117602.5`, `5714474287`, `5714892259.5` | correct — held | The overflow UID is a namespace-wide unmapped-owner value, not proof of host-root ownership. Source/generated copies and the namespace tests are left unchanged until the root/user security choice is answered; this row deliberately makes no completion claim. |
...
| `5714117602.4` (namespace wording) | better — documented | Changelog/ledger wording describes fail-closed namespace handling; no claim is made that an overflow UID is safe to trust while the choice is held. |
...
| `5714117602.5`/`5714474287` (S-01 overflow) and `5714117602.4` | correct — held | The security concern is valid, but root/user direction is pending. No source or generated installer copy is changed in this session. |

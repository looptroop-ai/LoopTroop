...
//
// `parseInt` rather than `Number.parseInt` for the same reason: they are the
// same function, but only one of them exists on a runtime old enough to need
// this message most.
...
//
// macOS gets the unversioned formula: `brew install node@24` is keg-only, so it
// installs Node and deliberately leaves it off PATH — the reader would run it
// and land back here.
function nodeHelp() {

...
| Greptile `4048988567` — tokenless startup cleanup could signal a recycled PID before Node observed child exit | FIXED | Tokenless readiness and cleanup now yield one event-loop turn before treating the direct `ChildProcess` handle as live, allowing pending exit notifications to settle. A regression proves a pending exit does not call `kill`; tokenless numeric fallback remains fail-closed. |
| Greptile `4046601297` / `4046844841` — Windows-only shutdown assertions are skipped or narrower | NOT A CRITICAL DEFECT | The affected assertions depend on platform taskkill/close semantics that are intentionally fail-closed when ownership cannot be proven; production Windows termination coverage remains in the platform matrix and no safety boundary is weakened. |
| Codacy `105690719136` — generic URL/path and process-token warnings | NOT ACTIONABLE | The URLs are built from the validated local daemon record, path inputs are constrained and realpath-compared before any mutation, and process tokens are local identity evidence rather than attacker-authenticated secrets. The provider supplied no reproducible behavior beyond these generic annotations. |

...
export const PROM10a: PromptTemplate = {
  ...
  instructions: [
    ...
    'Skip Reason Rule: A `## Why These Questions Were Skipped` section may list, for some skipped questions, the note the person left about why they skipped it. Treat those notes as evidence about intent and let them shape the answer you infer. They are read-only: there is no field for them in the output format, so never echo a note back, never copy one into `free_text`, and never treat one as the answer itself. A skipped question absent from that section was skipped without a note, which is not a signal about the answer.',
    CONDITIONAL_REPOSITORY_INSPECTION,
...
export const PROM_EXECUTION_SETUP_PLAN: PromptTemplate = {
  ...
  instructions: [
    ...
    'Backend-Owned Fields: Return only your proposal. LoopTroop supplies schema and ticket identity, artifact/status, current-host facts, temp roots, the ticket-locked project Git-hook policy and detected-hook evidence, quality policy, and derives readiness status/actions from the proposed work. The locked policy is copied exactly and any generated replacement is discarded. Do not echo or guess those fields.',
    ...
    'Git Hook Validation: Inspect repository hook configuration and propose explicit, safe validation commands for hooks you can identify. Do not invent commands for unknown hooks. The backend supplies read-only detected-hook evidence and the ticket-locked project policy; only validation commands are part of your editable proposal.',
...
export const PROM_EXECUTION_SETUP_PLAN_REGENERATE: PromptTemplate = {
  ...
  instructions: [
    ...
    'Backend-Authoritative Hook Policy: Preserve the ticket-locked project Git-hook policy conceptually, but do not copy or revise it; LoopTroop reimposes the exact locked value on the replacement plan while keeping hook-validation commands editable.',
    EXECUTION_SETUP_PROJECT_AGNOSTIC_RULE,

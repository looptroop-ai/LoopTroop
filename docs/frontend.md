...
AI log rows use consistent visible headers for prompts, reasoning, tools, and output. In combined AI views these render as `[PROMPT-<model-id>]`, `[THINKING-<model-id>]`, `[TOOL-<model-id>]`, and `[OUTPUT-<model-id>]`, using the complete configured model ID (including provider and router segments). Persisted canonical tags remain unchanged so historical records and internal extraction continue to work.

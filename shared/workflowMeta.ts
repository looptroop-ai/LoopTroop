...
const WORKFLOW_PHASE_DETAILS = {
  ...
  CODING: {
    overview: 'LoopTroop runs approved beads one at a time. The coding agent uses planned commands as guidance, runs the smallest sensible bead-scoped checks, and must return a structured all-pass completion marker. LoopTroop validates that marker without rerunning a frozen command list on its own. Each runnable bead records its reset anchor before publishing `in_progress`; a failed checkpoint leaves it pending so Retry can safely try again. Automatic bead-response continuations use the configured `maxIterations` cap within each bead iteration when it is finite; `0` means unlimited for that automatic path. User-facing Continue across workflow phases is separate. Deadline expiry and unresumable restarts go through Ralph reset-and-retry recovery. Local finalization failures become manual Retry or Cancel blocks. Bounded Git resets and commits preserve opaque path names and keep generated runtime files out of candidate commits. When all beads are done, Final Testing is still the required whole-ticket gate.',
    steps: [

...
if ($PSBoundParameters.ContainsKey('Version') -and [string]::IsNullOrEmpty($Version)) {
  throw '-Version needs a value, and was given an empty one.'
}
if ($PSBoundParameters.ContainsKey('Tarball') -and [string]::IsNullOrEmpty($Tarball)) {
  throw '-Tarball needs a value, and was given an empty one.'
}
if ($PSBoundParameters.ContainsKey('Prefix') -and [string]::IsNullOrEmpty($Prefix)) {
  throw '-Prefix needs a value, and was given an empty one.'
}

# `node` is looked up on PATH below, before the core and its resolver run.
# PowerShell never runs a command from the current location by name, but it
# does search every PATH entry, and an empty or relative one resolves against
# the current directory — the Downloads folder a one-liner is usually run from.
# The core's resolver never searches there; this is the same rule for the one
# lookup made before it exists, and the core and every tool it starts inherit
# the cleaned PATH. Absolute means what Node's `path.win32.isAbsolute` means:
# a drive and a separator, or a leading separator (UNC included), quotes and
# surrounding spaces aside.
$env:PATH = (@($env:PATH -split ';') | Where-Object { $_.Trim().Trim('"') -match '^(?:[A-Za-z]:)?[\\/]' }) -join ';'

if (-not (Get-Command node -ErrorAction SilentlyContinue)) {
# --- BEGIN node-floor (generated by scripts/sync-installers.mjs; do not edit) ---
  Write-Host 'LoopTroop needs Node.js 24.21.0 or newer and it is not on your PATH.' -ForegroundColor Red
  Write-Host ''
...
# --- END installer-core ---
  ...
  if ((Get-Item $corePath).Length -eq 0) {
    ...
    # The same way out install.sh offers. Without it this said what had gone
    # wrong and nothing about what to do next, which for a one-liner that
    # cannot be retried any differently is the whole of the useful advice.
    Write-Host 'Download it and run it from a file: https://www.looptroop.ovh/install.ps1'
    exit 1
  }

  $forwarded = @()
  if ($PSBoundParameters.ContainsKey('Version')) { $forwarded += @('--version', $Version) }
  if ($PSBoundParameters.ContainsKey('Tarball')) { $forwarded += @('--tarball', $Tarball) }
  if ($Binary) { $forwarded += '--binary' }
  if ($PSBoundParameters.ContainsKey('Prefix')) { $forwarded += @('--prefix', $Prefix) }
  if ($DryRun) { $forwarded += '--dry-run' }
  if ($Help) { $forwarded += '--help' }

  # So `-Help` prints the flags this wrapper takes rather than the ones
  # install.sh takes. The core renders one usage text in either dialect.
  $env:LOOPTROOP_INSTALL_STYLE = 'ps1'
